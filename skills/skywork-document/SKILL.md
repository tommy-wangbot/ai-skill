...
description: "Use when the task is to create a polished document in `docx`, `pdf`, `md`, or `html`, especially from prompts, reference files, or source materials. This skill authenticates to the Skywork Office Doc API, can parse input files first, and then generates a formatted document from a structured content prompt."
metadata:
  openclaw:
    requires:
      bins:
